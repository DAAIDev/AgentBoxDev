...
- [ ] **Rotate `GITHUB_TOKEN`** in AgentBoxDev's Cloud Run secrets. **Verified 2026-05-25**: Cloud Run secret `mcp-github-token` returns `Bad credentials` (HTTP 401) against `/user`. Triage worker can't run in production without a valid token. Same token is used by existing `kanban-github-sync.mjs` and `callGitHub` in `server.js` — they're all broken too in prod. **Probably skip in favor of the GitHub App** (ADR 0001) since we're going App-token anyway for Wedge 2 — but if any of the existing `callGitHub` paths in `server.js` need a working token before then, a stopgap PAT may be unavoidable.
- [ ] **Create GitHub App `boxai-self-repair-agent`** per ADR 0001 — browser flow at https://github.com/organizations/DAAITeam/settings/apps/new. This is Chris-only (org owner). Once created: install on `CRMBackend` + `CRMFrontEnd` (and optionally limit by repo); set `AGENT_GH_APP_ID` + `AGENT_GH_APP_PRIVATE_KEY` as Actions secrets in each CRM repo; mirror App ID/key into AgentBoxDev Cloud Run secrets so the triage worker can mint installation tokens too (replaces the dead `GITHUB_TOKEN`).
